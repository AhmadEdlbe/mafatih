...
router.post("/add", (request, response) => {
    ...
    database.query(queryCategory,productId, (error, result) => {
        ...
        if(result == "door handle"){
            order_number = '11' + util.getRandomInt(1000, 9999)
        }else if(result == "hardware"){
            order_number = '66' + util.getRandomInt(1000, 9999)
        }
...
router.get("/", (request, response) => {
    ...
    database.query(queryCategory,productId, (error, result) => {
        ...
        if(result === "door handle"){
            console.log('hello')
            order_number = 55 + getRandomInt(1000, 9999)
        }else if(result == "hardware"){
            order_number = 66 + getRandomInt(1000, 9999)
        }
